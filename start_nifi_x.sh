...
do_site2site_configure() {
  ...
  sed -i "s/nifi\.remote\.input\.host=.*/nifi.remote.input.host=${HOSTNAME}.nifi-headless/g" ${NIFI_HOME}/conf/nifi.properties
...
do_cluster_node_configure() {
  ...
  sed -i "s/nifi\.web\.http\.host=.*/nifi.web.http.host=${HOSTNAME}.nifi-headless/g" ${NIFI_HOME}/conf/nifi.properties
  sed -i "s/nifi\.web\.https\.host=.*/nifi.web.https.host=${HOSTNAME}.nifi-headless/g" ${NIFI_HOME}/conf/nifi.properties

  #sed -i "s/nifi\.security\.needClientAuth\.host=.*/nifi.security.needClientAuth=false/g" ${NIFI_HOME}/conf/nifi.properties
  #sed -i "s/nifi\.web\.http\.host=.*/nifi.web.http.host=dev-nifi.alar.net/g" ${NIFI_HOME}/conf/nifi.properties
  #sed -i "s/nifi\.web\.https\.host=.*/nifi.web.https.host=dev-nifi.alar.net/g" ${NIFI_HOME}/conf/nifi.properties

  sed -i "s/nifi.\web.\http.\port=.*/nifi.web.http.port=80/g" ${NIFI_HOME}/conf/nifi.properties
  #sed -i "s/nifi.\web.\https.\port=.*/nifi.web.https.port=443/g" ${NIFI_HOME}/conf/nifi.properties
  ...
  sed -i "s/nifi\.cluster\.node\.address=.*/nifi.cluster.node.address=${HOSTNAME}.nifi-headless/g" ${NIFI_HOME}/conf/nifi.properties
